...
public class ClimaService {
    ...
    @Value("${api.clima.geocoding.url:https://geocoding-api.open-meteo.com/v1/search}")
    private String urlGeocodingBase;

    @Value("${api.clima.forecast.url:https://api.open-meteo.com/v1/forecast}")
    private String urlForecastBase;

    public Map<String, Object> buscarDadosClimaAcesso(String cidade) {
        try {
            String urlGeo = urlGeocodingBase + "?name={cidade}&count=1&language=pt";
            
            @SuppressWarnings("unchecked")
            Map<String, Object> respostaGeo = restTemplate.getForObject(urlGeo, Map.class, cidade);

            if (respostaGeo == null || !respostaGeo.containsKey("results")) {
                throw new RuntimeException("Cidade não encontrada no mapa.");
            }
    ...
    public Map<String, Object> buscarDadosPorCoordenadas(String lat, String lon) {
        try {
            String urlClima = urlForecastBase + "?latitude={lat}&longitude={lon}&current=temperature_2m,relative_humidity_2m,apparent_temperature,precipitation,weather_code,wind_speed_10m,is_day&hourly=temperature_2m&daily=weather_code,temperature_2m_max,temperature_2m_min&timezone={tz}";

            @SuppressWarnings("unchecked")
            Map<String, Object> respostaClima = restTemplate.getForObject(urlClima, Map.class, lat, lon, "America/Sao_Paulo");

            if (respostaClima == null) {
                throw new RuntimeException("Falha na API de clima.");
            }
